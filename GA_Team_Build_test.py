...
class Individual:
    ...
    def return_random_player_by_position(self, position, available_price, player_id):
    ...
    def correct_non_feasible(self):
        ...
        while any(x is False for x in all_conditions):
            ...
            if len(result) == 0:
                ...
                new_player = self.return_random_player_by_position(duplicated_player.element_type, self.team_price(), duplicated_player.id)
                if new_player is None or not self.remove_player_and_add_new_player(duplicated_player, new_player):
                    continue
            ...
            while any(x < 0 for x in self.same_team):
                ...
                new_player = self.return_random_player_by_position(kick_player.element_type, self.team_price(), kick_player.id)
...
def mutation(individual):
    ...
    for i in range(length):
        ...
        while True:
            ...
            new_player = individual.return_random_player_by_position(kick_player.element_type, individual.team_price(), kick_player.id)
            if kick_player.id == new_player.id:
                continue

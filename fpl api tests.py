...
async def main():
    async with aiohttp.ClientSession() as session:
        ...
        for t in team:
            print(t.name,t.strength)

        players = await fpl.get_players()
        lista = [(x.code, x.web_name,x.total_points) for x in players]

        lista.sort(key=lambda tup: tup[2],reverse=True)

        with open('Last season total points.txt', 'w',encoding='UTF-8') as fp:
